...
public class GameWorld : MonoBehaviour
{
    ...
    private void LoadChunk(Vector2Int chunkPosition)
    {
        ...
        chunkData.Blocks = Generator.GenerateTarrain((int)xPos, (int)zPos);
    ...
    private void MouseCheckInput()
    {
        if (Input.GetMouseButtonDown(0) || Input.GetMouseButtonDown(1))
        {
            ...
            if (Physics.Raycast(ray, out var hitInfo))
            {
                ...
                {
                    Vector3Int chunkOriginal = new Vector3Int(chunkPos.x, 0, chunkPos.y) * ChunkRenderer.ChunkWidth;
                    if (isDestroing)
                    {
                        chunkData.Renderer.DestroyBlock(blockWorldPos - chunkOriginal);
                    }
                    else
                    {
                        chunkData.Renderer.SpawnBlock(blockWorldPos - chunkOriginal);
                    }
                }
            }
        }
